...
clean:
	rm -rf $(SYSROOT)
	cd bootloader && DESTDIR="$(SYSROOT)" SHAREDDIR="$(SHAREDDIR)" DEBUG_FLAGS="$(DEBUG_FLAGS)" $(MAKE) clean
	cd kernel && DESTDIR="$(SYSROOT)" SHAREDDIR="$(SHAREDDIR)" DEBUG_FLAGS="$(DEBUG_FLAGS)" $(MAKE) clean

build: clean
	cd bootloader && DESTDIR="$(SYSROOT)" SHAREDDIR="$(SHAREDDIR)" DEBUG_FLAGS="$(DEBUG_FLAGS)" $(MAKE) install
	cd kernel && DESTDIR="$(SYSROOT)" SHAREDDIR="$(SHAREDDIR)" DEBUG_FLAGS="$(DEBUG_FLAGS)" $(MAKE) install
