...
/*
  // Simple version:
  BootStatus = BS->GetMemoryMap(&MemMapSize, MemMap, &MemMapKey, &MemMapDescriptorSize, &MemMapDescriptorVersion);
  // Will error intentionally
  BootStatus = BS->AllocatePool(EfiLoaderData, MemMapSize, (void **)&MemMap);
  ...
  BootStatus = BS->GetMemoryMap(&MemMapSize, MemMap, &MemMapKey, &MemMapDescriptorSize, &MemMapDescriptorVersion);
  BootStatus = BS->ExitBootServices(ImageHandle, MemMapKey);
*/
